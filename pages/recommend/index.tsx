...
const Recommend: NextPage<IProps> = ({ isPc, bookInfos = [], page, pages, bookPackageId }) => {
  useEffect(() => {
    // 点击浏览器刷新，返回2级书籍列表顶部
  ...
  return <>
    ...
    {isPc ? <PcRecommend page={page} pages={pages} bookInfos={bookInfos} bookPackageId={bookPackageId} /> :
      <MRecommend page={page} pages={pages} bookInfos={bookInfos} bookPackageId={bookPackageId}/>
    }
  </>
}
...
export const getServerSideProps: GetServerSideProps = async ({ req, query, locale }): Promise<GetServerSidePropsResult<IProps>> => {
  ...
  const { page = "1", bookPackageId } = query;
